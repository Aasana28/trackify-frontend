...
export default function MockInterviewPage({ applications }) {
  ...
  // ── Step 2: Submit answer, get AI feedback (stays on same question) ───────
  ...
  // ── Step 3: Move to next question (or finish) after reviewing feedback ────
        ...
        <div className="card mock-session-card">
          {!session ? (
            ...
            <div>
              ...
              {session.feedbacks[currentQ] === undefined ? (
                <>
                ...
                <>
                  <div style={{ fontSize: "0.82rem", fontWeight: 600, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 6 }}>Your Answer</div>
                  <div style={{ fontSize: "0.88rem", color: "var(--text-secondary)", background: "var(--bg-primary)", borderRadius: 8, padding: "10px 14px", marginBottom: 16, border: "1px solid var(--border)" }}>
                    {session.answers[currentQ]}
                  </div>

                  {(() => {
                    const fb = parseFeedback(session.feedbacks[currentQ]);
                    return (
                  ...
                  <div className="mock-actions">
                    <button className="btn btn-primary" onClick={nextQuestion}>
                      {currentQ + 1 === session.questions.length ? "Finish Session" : "Next Question →"}
                    </button>
                    <button className="btn btn-ghost" onClick={resetSession}>End Session</button>
                  </div>
                </>
              )}
            </div>
